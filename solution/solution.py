...
def run_assistant(
    ...
    while True:
        ...
        turn_tokens = count_tokens(user_msg) + count_tokens(reply_text)
        turn_cost = estimate_cost(user_msg, reply_text, model=OPENAI_MODEL)["total_cost"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    table += "-" * 100 + "\n"
    return table + "\n".join(
        f"{res['prompt'][:40]} | {res['gpt4o_response'][:40]} | {res['mini_response'][:40]} | "
        f"{res['gpt4o_latency']:.2f} | {res['mini_latency']:.2f}"
        for res in results
    )
